...
export class CustomInstructionsController {
  ...
  constructor(
    private readonly customInstructionsService: CustomInstructionsService,
  ) {}
  ...
  async create(
    ...
    try {
      ...
      this.logger.error(
        `Error creating custom instructions for user ${req.user.userId}:`,
        error,
      );
  ...
  async findAll(@Request() req: any): Promise<CustomInstructionsResponseDto[]> {
    try {
      this.logger.log(
        `User ${req.user.userId} fetching all custom instructions`,
      );

      return await this.customInstructionsService.findAll(req.user.userId);
    } catch (error) {
      this.logger.error(
        `Error fetching custom instructions for user ${req.user.userId}:`,
        error,
      );
      throw new HttpException(
  ...
  async findActive(
    ...
    try {
      this.logger.log(
        `User ${req.user.userId} fetching active custom instructions`,
      );

      return await this.customInstructionsService.findActive(req.user.userId);
    } catch (error) {
      this.logger.error(
        `Error fetching active custom instructions for user ${req.user.userId}:`,
        error,
      );
      throw new HttpException(
  ...
  async findOne(
    ...
    try {
      ...
      this.logger.log(
        `User ${req.user.userId} fetching custom instructions ${instructionId}`,
      );

      return await this.customInstructionsService.findOne(
        instructionId,
        req.user.userId,
      );
    } catch (error) {
      this.logger.error(
        `Error fetching custom instructions ${id} for user ${req.user.userId}:`,
        error,
      );
  ...
  async update(
    ...
    try {
      ...
      this.logger.log(
        `User ${req.user.userId} updating custom instructions ${instructionId}`,
      );
      ...
      this.logger.error(
        `Error updating custom instructions ${id} for user ${req.user.userId}:`,
        error,
      );
  ...
  async remove(
    ...
    try {
      ...
      this.logger.log(
        `User ${req.user.userId} removing custom instructions ${instructionId}`,
      );

      await this.customInstructionsService.remove(
        instructionId,
        req.user.userId,
      );
      ...
      this.logger.error(
        `Error removing custom instructions ${id} for user ${req.user.userId}:`,
        error,
      );
